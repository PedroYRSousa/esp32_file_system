...
static void test_file_open_and_write_all_MustBeReturnESP_OK(void)
{
	...
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(file_path, texts, mode, true), ESP_OK);
...
static void test_file_open_and_write_all_MustBeReturnESP_ERR_INVALID_ARG(void)
{
	...
	TEST_ASSERT_EQUAL(esp_vfs_spiffs_register(&fs_conf), ESP_OK);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(NULL, texts, mode, true), ESP_ERR_INVALID_ARG);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(file_path, NULL, mode, true), ESP_ERR_INVALID_ARG);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(file_path, texts, NULL, true), ESP_ERR_INVALID_ARG);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(NULL, NULL, NULL, true), ESP_ERR_INVALID_ARG);
	TEST_ASSERT_EQUAL(esp_vfs_spiffs_unregister(fs_conf.partition_label), ESP_OK);
}
...
static void test_file_open_and_write_all_MustBeReturnESP_FAIL(void)
{
	...
	TEST_ASSERT_EQUAL(esp_vfs_spiffs_register(&fs_conf), ESP_OK);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all(file_path, texts, mode, true), ESP_FAIL);
	TEST_ASSERT_EQUAL(fs_file_open_and_write_all("test", texts, mode, true), ESP_FAIL);
	TEST_ASSERT_EQUAL(esp_vfs_spiffs_unregister(fs_conf.partition_label), ESP_OK);
}
